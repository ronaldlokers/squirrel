...
**Architecture:** Every message is still spooled and drained into Postgres, and Postgres stays off the request path exactly as reviewed. Three things are added. The 🐿️ receipt becomes a **boost** — a reaction on the message rather than a reply — which needs no configured credential because `room.path` carries the bot key. An **applier** runs after each insert, re-runs a pure intent matcher over the stored raw text, executes the intent and posts its answer through `Send()`. A **scheduler** posts one digest a day, idempotent by local date.

Task 1 is independent of the other ten: boosts need no bot key, no database and no scheduler, so it can ship on its own if the rest slips.
...
- **Postgres is never read or written on the request path.** The sink, spool and HTTP server are not modified by this plan at all. If a task finds itself editing `sink.go`, `spool.go` or `http.go`, stop and report. Task 1 is the sole exception to the transport being untouched, and it adds no database access.
- **The `Content-Type` contract is unchanged.** `ignored` sends no header, an unrouted path sends none, a panicking handler sends none, and `failed` is a 200 with `text/plain`. Task 1 changes only what `stored` does, and it changes it *to* the same shape as `ignored`.
...
### Task 1: Receipts become boosts

**Files:**
- Modify: `internal/transport/campfire.go`
- Test: `internal/transport/campfire_boost_test.go`, `internal/transport/campfire_test.go`

**Interfaces:**
- Consumes: `squirrel.Outcome`, `CampfirePayload` (phase 1)
- Produces:
  - `func BoostURL(baseURL, roomPath, messageID string) string` — package-private is fine, exported here so the test can assert the shape directly
  - `Respond` changes: `Stored` now answers with no `Content-Type`
  - `NewCampfire`'s handler fires a boost after a `Stored` outcome

This task is independent of every other task in the plan. It needs no bot key, no
database and no scheduler, because the key rides in on `room.path`. It can ship
on its own.

**The `Content-Type` contract is unchanged and every existing test for it still
applies.** `ignored` and the panic path and the silent 404 are untouched. What
changes is that `stored` now takes the same shape as `ignored` — a 200 with no
header — which is correct: both post no message, and the difference the user sees
is the boost.

- [ ] **Step 1: Write the failing test**

`internal/transport/campfire_boost_test.go`:
```go
...
- [ ] **Step 2: Amend Task 7\'s response test in `campfire_test.go`**

`TestRespondStored` currently asserts `text/plain` and a 🐿️ body. The contract
has changed: replace its body with
...
`TestRespondIgnoredSendsNoContentType` and `TestRespondFailedIsStill200` are
unchanged and must stay passing. `TestCampfireStoresACapture` asserts the body is
`"🐿️"` — change that assertion to an empty body and no `Content-Type`; the rest
of the test stands.

- [ ] **Step 3: Run to verify they fail**

Run: `go test ./internal/transport/`
Expected: FAIL — `undefined: transport.BoostURL`, plus the two amended tests
failing against the old `Respond`.

- [ ] **Step 4: Write the implementation**

In `internal/transport/campfire.go`, change the `Stored` case of `Respond`:
...
Add the boost, above `NewCampfire`:
...
In `NewCampfire`\'s handler, after computing the outcome and before `Respond`,
capture what the boost needs; then fire it once the response is written:
...
`boostClient` is a `&http.Client{Timeout: 5 * time.Second}` created once in
`NewCampfire`, alongside the one `sendVia` already builds.

`CampfireConfig.BaseURL` is currently forced empty when no bot key is set — see
`campfireFrom` in `config.go`. **That must change**: the base URL is now needed
for boosts, which require no key. Remove the `if botKey == "" { baseURL = "" }`
line, and adjust `TestLoadConfigLeavesSendUnconfigured` to assert only that
`BotKey` is empty. `Send` is still nil without a key — that logic is in
`NewCampfire` and is unchanged.

- [ ] **Step 5: Run**

Run: `go test ./internal/transport/ -v && make check`
Expected: the 7 new tests and all 19 existing ones PASS.

- [ ] **Step 6: Commit**

```bash
git add internal/transport internal/squirrel/config.go internal/squirrel/config_test.go
git commit -m "feat: acknowledge a capture with a boost rather than a message"
```
...
### Task 3: Duration and chore-definition parsing
...
### Task 5: Chore storage and the due query
...
### Task 6: Events, and proving the derived clock
...
**Interfaces:**
- Consumes: `Chore`, `DueChores` (Task 5)
- Produces:
...
### Task 7: Prompts and their numbering
...
### Task 11: Boot wiring, and the homelab change
