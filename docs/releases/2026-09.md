...
### v0.84.0 — 11 September 2026

**Every mood face has a second drawing, and the day chooses.**

The five faces were the last thing on the board drawn the same way every day,
on the surface that most needs to keep being seen. Each now has a second
drawing beside it: same colour, same tile, same outline and the same kind of
furniture, with only the expression moving. Good opens its eyes, calm looks up
under a single sprout, low looks down under a raincloud, frazzled raises worried
brows beside a bolt and sweat drops, and wiped droops beside a nearly empty
battery.

The date picks between the two, the way the sentences and the stamp's angle
already do, so the dial, the face in the ops bar and the five check-in presses
agree all day and a reload is not a slot machine. With only two drawings the
pick mostly alternates day to day, and good, calm and wiped move together.

The drawings came from an image model given each shipped face as a style-only
reference, cut out and placed with `scripts/cutout.sh`, which is new in this
release too. Two were drawn twice because the first attempt left the mood:
frazzled came back angry and wiped came back unimpressed. The alternates had
been declined on 23 August; the art record now says why they were revisited.

No new screen, no new flow. No schema change: the rollback is the image tag
alone.

---

### v0.83.0 — 9 September 2026

**Looking for something on a phone showed a blank screen.**

Search is in the bar on every screen, and on a phone it led nowhere. The page
was rendering the matches, the count and the way back the whole time and
drawing none of them: on a 390px phone the whole document read `R`.

*What matched* is not a fifth bay. By design it takes the racks' own place —
one rack where the four normally are — so it wears the racks' class. When the
phone became the rail rather than the racks, the rail's layout hid the racks
below 620px with a blanket rule, and took the one rack that is not a rack with
them. The rule now spares it. The phone keeps the rail, and keeps what matched:
full width, the way back at the top, scrolling in the deck the way the rail
does.

**The lid is closed, and now something says so.** The devices spec stopped the
bar at three icons on 22 August, after a fourth wrapped it to a second row —
which does not merely look wrong, it doubles the height of the one band the
reach zone gives the least room to. That has been a decision with nothing
enforcing it ever since. A test now fails if a fourth appears, if the find field
falls under 120px, or if the bar holds more than it fits, at four phone widths.

**The Devices roadmap item is reconciled** against the five acceptance criteria
its spec set on 22 August, before the conversation retired. Four are met and
the roadmap says which test holds each. The fifth — every screen's primary
action inside the thumb band from a cold open — is left open on purpose: the
rail's head answers sit at the reach/stretch line because the comps put the head
at the top, so meeting it is a change to an approved comp rather than a fix.

No schema change: the rollback is the image tag alone.

---

### v0.82.0 — 9 September 2026

**The board on a tablet, and a count that counted the wrong thing.**

A tablet is a desk you hold, and it was getting the desk's layout at half the
desk's width. Four racks and a sidebar need about 1240px before a rack is wide
enough to read a chore beside its mark; an iPad has 1180 in landscape and 834 in
portrait, so every rack was about 170px and words were being cut mid-letter.

Between 621 and 1239 the four racks are **two across**, in a square, with the
**sidebar as a band** under them. The deck scrolls rather than each channel:
four channels with their own scrollbars is four places to lose your position on
a screen you are holding.

Two fixes underneath that hold at every width. **A strip measures itself against
its rack rather than the window** — below 265px of rack the mark drops under the
words instead of taking a `max-content` column beside them, which is a container
query, because no viewport breakpoint knows a rack's width. And **a word with
nowhere to break breaks anywhere** rather than running out of a strip that clips
it. The bar sheds the date below 1000px and the wordmark below 820px so the find
field never drops under the width it needs.

**The notes count said 41 where the wall showed 2.** What a note is is decided in
Go — the read drops any row the matcher reads as a command — and every `!at` and
`!action` typed in Campfire is stored before it is applied, because capture is
sacred. A `count(*)` over the same clause counted all of them. Both numbers come
from the same read the wall uses now.

No schema change: the rollback is the image tag alone.

---

### v0.81.0 — 9 September 2026

**The body double is retired.**

Timers are gone from the product — not moved, removed. No countdown on any
screen, no `!timer`, no `!start`, no `!stop`, no `start_timer` tool for the
coach, and no push that says *time*. What it was for, the product does with its
rules rather than with a clock, and a clock that has to be watched is one more
thing to manage.

Four things existed only because timers did, and went the same day: **the
breadcrumb**, which was made from a finished timer; **the exit ramp**, which was
a property of a running one; **the stuck ladder's one control**, whose four
sentences stay — *boring* now says what its alarm used to, a short go ended by
you rather than by a bell; and the coach's **`typically`** fact, measured from
finished runs.

`!stuck` and everything else stays. The `timers`, `timer_runs` and `exit_ramp`
tables are untouched — no migration, nothing dropped, no rows deleted — so the
rollback is the image tag alone.

---

### v0.80.0 — 9 September 2026

**The board as it was drawn.**

v0.79.0 shipped the redesign from a written summary of what had been decided
rather than from the comps themselves, and the two had drifted badly. This is
the same design, built against the drawings.

**The pick has no card.** The picker still chooses one thing and still says why,
but on the row the pick is about — `PULLED` and the rule that chose it, above
the name — rather than on a card at the top of a board that was already showing
that thing. `not this one` and `I'm stuck` sit quietly under the row's stamps,
with the blocker ladder in the same place.

**The running timer's band went with it.** A countdown hangs on the row of the
thing it times. Three things have no row of their own and get a loose one at the
head instead: a timer whose thing is not on the board, the breadcrumb, and a
pick that is further back than a rack draws.

**The way in is a field, not a button.** Floating at the foot of the board.
Enter keeps a thought — one keystroke. The `+` beside it opens the writer
carrying what you typed.

**The writer is one screen of pills.** Kinds as a row, and the kind's own fields
in a nested panel as presets: *every day* · *every week* · *2 weeks* ·
*a month*. A number box with a unit dropdown was two decisions where the answer
is nearly always one of four.

**The phone's head is its own object** — paper, the reason above the name, and
two large stamps. It is the only place on the phone a thing can be answered.
Everything else today hangs off the rail with a time block and nothing else.

**Off the rail is two rows with counts** — *things you decided* and *in the
notes*. A chore that is not asking today is no longer on the phone at all; it is
on the desk, in its rack. The counts overrule principle 2's surviving clause on
the owner's decision, with three rails recorded in `PRODUCT.md`.

**And the defect underneath all of it.** The service worker was re-issuing every
capture POST with the path only, dropping the query string — so a form carrying
a field in its action lost it silently. The post landed, the handler found
nothing to do, and the screen looked like it had worked. Found by driving the
app rather than reading it, which is what `walkthrough_browser_test.go` now does
on every writer, in a real browser, on every run.

No schema change: the rollback is the image tag alone.

---

### v0.79.0 — 9 September 2026

**The desk becomes one board, and the phone becomes its own screen.**

Eight changes in a day, from one design conversation. The largest is that the
board stopped being four kinds of thing in four places and became one thing cut
by how often it comes back — and that the phone stopped being a fold of that
board.

**The board says which chores are asking for you today.** A chore at rank one to
four sits on paper, lifted, above a seam; the rest of the rack is below it,
carrying its name and its rhythm and nothing else. Nothing is dimmed. The rack's
sign counts what is asking, not what it holds.

**What is coming moved into the sidebar** and the agenda stopped being a door.
Every fixed point still ahead, soonest first, the time set as a deadline in
printed figures and the leave-by underneath it as arithmetic. One inside its
leaving window hoists above the dial. The cost is recorded: the diary is read
now rather than worked, and the agenda's four answers went with its door.

**The two things the world put a time on can say they are late.** A fixed point
that has started stays for two hours and says so; a chore the world put on a
weekday says `late — today` on that weekday and leads its rack. A chore on an
interval never does — that is a rhythm you set yourself, and Squirrel does not
invent a time you can be late for. It says late and never how late, and it never
survives the day that caused it.

**A task is a chore whose rhythm is once.** The tasks became the fourth rack,
after seldom, because the columns run by how long until a thing returns and a
once-thing never does. The one you decided last is what asks for you. The row of
doors under the racks went with it, and yellow retired from the palette: a chore
and a once-thing are one kind now.

**The notes are a wall.** `/notes`, its own page, every note you have with its
photograph drawn at the width of its column, and two presses — *drop*, and *make
a chore*. No triage, no kept, no set aside. The shelves, the ledge and the
settled seams are gone, and so is Campfire's `keep N`, because the board has no
shelf to keep something on.

**One button adds anything.** Three writers asking *what is it* in three places
became one modal, opened from a bar under the racks, asking the words before the
kind. Four kinds and only the fields that kind needs. Nothing is written until
*keep it*, and closing it keeps nothing.

**A fixed point can come round.** Every week, every two, every four. Not a rule
engine: the next one is made when you keep the one before it, so a diary ignored
for a month holds one appointment rather than four.

**The phone is a rail.** Today, top to bottom, in the order today happens, with
the one that wants you now at its head and everything else hanging off the line
at its hour — or at the part of the day, when that is all there is. Off the rail,
whatever has no hour. The tab bar went; the writer took the foot.

Two migrations, so the rollback is **not** the image tag alone.
`0043_notes_are_a_wall.sql` opens every note that was kept, waiting, blocked or
someday; `0044_a_fixed_point_can_come_back.sql` adds `every_weeks` to `moments`.
Both are forward-only and neither drops anything: going back to v0.78.0 leaves
the notes open and the column unread.

---

### v0.78.0 — 8 September 2026

**The board becomes the chores.**

Four bays of four different kinds of thing asked, every time the board was
...
list of tasks, which is the shape that goes invisible. This release answers
that, and it is the largest change to what the board *is* since the room
retired.

**Chores have a rhythm.** Daily up to two days, weekly up to a fortnight,
seldom beyond it. A monthly thing and a daily one stop being the same kind of
row.

**Chores have a usual time**, read from the completions already in `events` —
no new capture, no migration. The newest twelve, in your own clock, and each
half claimed only when it holds more than half of them: a chore you do every
day has no usual weekday and keeps its morning. Fewer than three completions
claims nothing at all.

**Chores have an order Squirrel can explain**, six rules in a fixed order, and
every row carries the sentence that puts it there. It comes back today and this
is when you usually do it · it comes back today and you usually do it later on ·
it comes back today · not yet, but this is the day you usually do it ·
everything else · never done. This is principle 6 taken from the picker's one
offer to every row on the board.

A chore never done is last and is never called due. A thing created a year ago
and never started is not three hundred and sixty-five days overdue.

A row whose turn is simply not today says nothing at all — and above all not how
long it has been waiting. The rack has refused to print that since it was built.

**The board is three racks and three doors.** Daily, weekly and seldom across
the board with one writer under them; the notes, the tasks and the agenda become
doors beneath. A door opens onto what it holds in the racks' place, whole, with
a way back, the same as search and the shelves already did.

*What that costs, plainly:* the pile is no longer visible without a press. Kept
because the pile is where you go when you have decided to triage, and the chores
are what has to be seen without deciding to look. `PRODUCT.md` records the trade
and how it reverses.

**Mood is on the board.** A ring of seven arcs — the week behind you, today
closing the circle — around today's face, then the five faces, then the whole
record. The faces are always pressable: saying how you are stopped being
something you wait to be asked for. Being asked is still separate and still
keeps principle 9. *Mood charts and trends* left the refused list to allow it,
and what shipped is not a chart: nothing is totalled, averaged, compared or
trended, and the days you said nothing are drawn as gaps.

**On a wiped or frazzled day the racks hold what comes back today and count the
rest.** The count is never phrased as a backlog, and a browser test reads the
sentence for the words that would make it one. Not on a low day, for the reason
the capacity gate already gives.

**The picker stopped offering chores.** Six rules became five. An offer naming a
chore could only repeat the top of a rack the same screen was already showing.
Chores still reach Campfire through the nudge, which is untouched.

**On a phone** the racks become four tabs — now, daily, weekly, seldom — where
*now* is the cut the desk has no use for: everything with something to say for
itself today, across all three racks, in one order. The doors sit in a row above
the bar and the dial folds behind today's face in the ops bar.

The four drawn bay icons are deleted rather than left unreferenced: four cuts of
one list are not four things, and there is no drawing of *seldom*.

Merged as #335, #339, #337 and #338. Every claim in this release was proven by
reverting the behaviour and reading the assertion that failed. Two of those
mutations found holes rather than confirming coverage — a mood test that watched
the form rather than the faces, and the dial's ring, which could be cut out with
the whole suite still green. Both were fixed before the merge.

No migration.

---

### v0.77.0 — 8 September 2026

**The four findings the review left for the owner to decide.**

v0.76.0 shipped twenty-four of the second review's twenty-eight findings. The
other four each needed a product call rather than an implementation. All four
were answered on 8 September and are here.

**The hyperfocus exit ramp runs.** It was fully built and wired to nothing — a
migration, a column, a hush, four store methods and an integration test, with no
callers. A timer started from the screen now offers to catch you if you lose
track; half an hour after it runs out, the board asks once, and you can stop,
keep going, or say leave me alone for the rest of the day. It is silent between
22:00 and 06:00 and on a low day, checked before anything is read, and neither
can be lifted.

Wiring it turned up that `ended_at` was carrying two meanings — *you stopped it*
and *the tick announced it* — and the ramp only ever wanted the first. Stopping
a timer now clears the ramp, which is what pressing stop means: you noticed,
which is the thing the ramp exists to supply.

**An opened strip says what was noticed about it before.** The rack still draws
one line per thing, because two lines under one strip is a conversation and a
rack is not one. What changed is that the lines before it are kept rather than
overwritten, and the strip you open reads them back. *What did it tell me about
the boiler last week* had no answer anywhere before this.

Two things fell out of it: opening a strip was losing the line the rack had been
showing, and a new observation was reviving a refusal you had already made.
Refusing said do not say this again, and it stays said.

**Every control clears 44×44 on a phone.** The chore's rhythm and the
appointment's day and clock were words inside a sentence and under the floor,
and the exemption said giving them the floor was a redesign of that row. It is
that redesign: they stay a sentence above 620px and become boxed fields below
it. The lid's search icon takes the room a flex spacer was holding. **The
exemption mechanism is deleted**, so nothing can be quietly excused from the
floor again — and one of the three exemptions turned out to be for a control no
template has had since the room retired.

**Buddy is told what did not land again.** The prompt has always asked for
examples of answers that landed badly, and the press that produced them went
with the room on 6 September, so it was asking for something nothing could
supply. It reads what *not useful* refused on the board instead — the same
signal, since the lines it refuses are Buddy's own words.

**Migration 0042.** The rollback is not the image tag alone.

---

### v0.76.0 — 7 September 2026

**Everything the second product-team review found, less what needs a decision.**

A cross-functional review on 6 September read the product as it stood after the
room retired, with independent verification of every bug-class finding because
the same party wrote and reviewed most of the code. Twenty-four of its
twenty-eight findings are in this release. The four that are not each need a
product call rather than an implementation.

**Nothing you said is lost.** Four separate ways it could be, all closed. A
capture typed on the board with no network went to a route the service worker
did not know, so the browser's own error page replaced the board and the words
went with it — the worker knows both board routes now and holds the whole form,
so a note typed on the chores tab goes back to the chores when it lands. A
photograph Squirrel will not take no longer takes the words with it: it is a
refusal, and the words come back into the box with the reason. A photograph you
have chosen is held on the device again, which `PRODUCT.md` has promised since
August and which went with `pile.js`. And a volume that will not write is its
own error now, so a working camera is never reported as the wrong kind of
picture.

**Asked for, never guessed at.** The chores field shipped `value="7"`, so the
branch that asks how often something comes back could not run and a thought
typed on that tab became a weekly chore. Seven is a hint now.

**The board can be overwhelmed.** The escalation to the deep model reads what
was said, and the board's ask was sending a fixed phrase — so the one turn that
model exists for was reachable only from Campfire.

**The board answers to its own letters.** D, K, X, R, W, B, Y on the opened
strip, D, N, W, S, T on the pulled strip and Z on the shelf were drawn and did
nothing. A key hint that lies teaches a way of working that stops working when
you move.

**A screen that says what it did.** An answer you asked for is something focus
is moved to, so a screen reader reads it. Every field says what it is. `not
useful` is lowercase, as the law always said, and *ask Buddy* is drawn as paper
beside the answers rather than as a fifth disposition — and it names who
answers before you spend a call.

**One stylesheet.** `pile.css` is deleted: 2,006 lines of the retired room that
`/me` and the way in went on loading. Both are drawn from the board's own
tokens. 913 lines and 145 of chrome, against 2,861.

**The ops bar has a middle.** It lost one when Buddy's chip went and the right
half kept its place.

**The one count this product permits is on a screen again.** What the coach has
cost this month went with the coach sheet; it is on the page about you, and a
test walks every bay to keep it there and nowhere else.

**Operationally.** Both architectures are built and started on every CI run
rather than arm64 first appearing during the release that ships it — the
Dockerfile cross-compiles, so a clean arm64 build is seconds. Migration boot
checks applied versions in one round trip instead of one per migration. One
board render no longer asks a ten-connection pool for twelve. A failed read of
the steps or the timer says so instead of looking like nothing to show. Where a
login returns to is parsed rather than pattern-matched, so a tab or a newline
cannot walk out of the host. The board's today is read where you are, in the
code rather than in an env var.

**Privacy.** A photograph chosen and never kept is dropped when you sign out —
nothing else ever cleared it. Held captures are not, deliberately: they are the
only copy, and `PRODUCT.md` now states that and its cost. `coach_answers` keeps
the person's half of the turn whole, including what was on screen, which left
the machine and was in no row.

**The records.** Four binding documents still described the room. A sweep now
fails the build when one names a route that is not mounted; it found a fifth on
its first run.

No migrations. The rollback is the image tag.

---

### v0.75.0 — 6 September 2026

**The room retires, and the board is the whole app.**

The decision of 3 September, finished. The conversation inside the app is gone;
there is the board, and Campfire beside it.

**16,274 lines went.** The turns did not: the record is kept and what went is
the thing that drew it. Campfire is untouched — nothing in the transport imports
the screen, and the five things `PRODUCT.md` says chat keeps forever are all
Campfire's own.

**Steps came over first**, onto the opened strip, the way the strip's own
corrections did in v0.73.0. That was phase E, and it was the last thing still
reachable only from the room.

**Two things the deletion broke, and the browser suite caught.** Both were real
rather than stale, and both are worth recording because a deletion this size is
exactly where a silent regression hides.

`pile.js` registered the service worker and did the push subscription, and it
was deleted with the room — so nothing registered a worker at all. Offline
capture, the held capture's flush and the way to turn notifications on all died
together, while the button to turn them on sat in the page with nothing left to
unhide it. That layer is `app.js` now, loaded by the board and by every page on
the layout.

And the focus-ring sweep tabbed to the dock, which no longer exists. It measures
the blank strip instead, the other cream surface a key can reach.

**What the deletion cost, stated rather than buried.** The browser tests that
measured the room's own shapes went with it — the dock, the gutter, the
transcript, the faces line, the slot, the turn-card stock. Their selectors match
nothing in the surviving templates, so they could not be repointed; pointing
them at the board would have been a test that passes without testing.
**Photograph capture has no browser test any more.** The board still has the
camera, and that test is the next one to write.

**Also: the phone's second note box is removed**, four days after it shipped. It
was pinned above every tab but the notes so a thought typed on the chores tab
could not become a weekly chore by accident. Two boxes on one screen asking
different questions is the same decision it was meant to spare you, made twice
instead of once, and it is the third capture surface the refused list already
names.

The problem it was aimed at is real and stays open, and is written into the
refused list rather than quietly closed: the chores field defaults to every
seven days, so words it was not expecting still become a rhythm. That is for the
field to fix, not the screen.

**No migrations.** The previous image runs against this schema, so the rollback
in `docs/running.md` is a tag change and nothing else.

### v0.74.0 — 5 September 2026

**What Squirrel knows about you is legible.**

Six things it had worked out were drawn on the page about you as six empty cream
boxes. The rows were there — six of them, between 49 and 76 characters — and the
words were rendering. Only the colour was missing.

`.knows li` asked for `var(--ink)`, and nothing in this product has ever defined
`--ink`. There is an `--ink-soft`; there has never been a bare one. An undefined
custom property makes the whole declaration invalid, so the colour fell back to
what the purple page inherits, which is cream, on a near-white card.

It shipped in v0.68.0 and had been on the phone ever since.

**Nothing caught it because the page only draws those cards when there is
something to show**, and the dev screen answered that question with nothing. The
empty state is what everyone looked at, including an accessibility audit of this
page that found its heading structure and missed its text. The owner found it by
looking at his own phone.

The dev screen knows three things now. A surface that cannot be seen in
development is a surface nobody reviews.

**And a test for the class rather than the instance.** Every stylesheet is read,
its comments stripped, and any custom property used but never defined — in a
sheet or on an element in a template — fails it.

Writing that test correctly mattered more than writing it. The first version
fired on six properties and five were its own fault: `--foot` and `--mood-calm`
are defined mid-line inside `:root { … }`, which a line-anchored pattern misses,
and `--railw` appears only inside a comment. A test that is wrong five times out
of six is worse than no test, because the sixth time stops being believed.

### v0.73.0 — 5 September 2026

**The edges the review left, and one decision reversed before it shipped.**

The last of the findings, and the release where refusing was as much of the work
as building.

**The board draws faster.** Its render was ten-plus database round trips in a
row, so its latency grew with how many things the board *has* rather than with
how much is *on* it — on a single-replica Pi. The independent reads run
concurrently now, and a single-bay draw stops reading both shelves it cannot
show. About 260ms of waiting becomes under 100ms against a store with 20ms on
every call.

**Capture stopped being able to keep a thought twice.** A retry after a dropped
response — the write landed, the acknowledgement did not — wrote the row again.
The key is made once in the service worker before the first attempt, so it
survives into the held record and comes back unchanged on the flush, which is
exactly the case that produced the duplicate.

**A third answer on the pulled strip.** v0.69.0 took away the way to say the
offer itself was wrong and left *not today* carrying both *wrong pick* and
*right pick, wrong moment*.

**It was built to suppress permanently, and that was reversed before merge.** A
chore pressed once would have left the picker for good — still in its bay, still
with its rhythm, nothing showing why and no way back from any surface. That is
the product quietly forgetting something it exists to remember, and it makes
Principle 6's *fixed, readable rules* unreadable. The finding asked for an
answer recorded distinctly from a deferral, and recording distinctly is not
hiding forever. It suppresses for the day, like *later*; tomorrow is a fresh
question for this answer too.

**The phone is walked rather than guessed.** The two mechanical acceptance
criteria the devices spec settled on 22 August finally exist: every control
clears 44×44 at a phone viewport with touch emulation actually on, and nothing
scrolls sideways at 320px. **Seven controls were four pixels under the floor**
and nobody had measured — the bar's chips, the strip's chevron and its way back,
the blank strip's field and its `+`, and the search pill.

Three more are knowingly below it and now say so by name rather than being bent
to fit: the chore rhythm and the appointment's day and clock are words in a
sentence rather than buttons; the lid's search icon cannot grow without pushing
the field beside it under the same floor; and the compose field's rest height is
what the dock's reserve is computed from.

**The overflow test nearly shipped unable to fail.** It was specified against
the document's own scroll width, and `html, body` carry `overflow-x: clip`,
under which that can never exceed the client width. Proved by mutation, and
moved onto `body`, which does track it.

**The supply chain and the record.** Every third-party action is pinned to a
commit rather than a mutable tag — the image job holds `packages: write` — and
each SHA was resolved through the API and verified again before merge. `:latest`
is no longer pushed: the build is not reproducible, so it pointed at an image
whose digest could not be tied to any version, and nothing deployed reads it.

And the roadmap went from **2,581 lines to 264**. The 56 shipped entries moved
here, verbatim and counted on both sides. The roadmap keeps what its own first
line always claimed it was: state, not history.
